...
//Function via http://detectmobilebrowser.com
//TODO: User should have some kind of option to toggle this on/off.
function isMobile() {
	$ok = FALSE;

	//UserAgent header can be omitted so make sure it exists
	if(!empty($_SERVER['HTTP_USER_AGENT'])) {
		$userAgent = $_SERVER['HTTP_USER_AGENT'];
		$ok = preg_match('/(android|bb\d+|meego).+mobile|avantgo|bada\/|blackberry|blazer|compal|elaine|fennec|hiptop|iemobile|ip(hone|od)|iris|kindle|lge |maemo|midp|mmp|mobile.+firefox|netfront|opera m(ob|in)i|palm( os)?|phone|p(ixi|re)\/|plucker|pocket|psp|series(4|6)0|symbian|treo|up\.(browser|link)|vodafone|wap|windows ce|xda|xiino/i', $userAgent)
		      ||
...
function _get_themelet_files($_theme) {
	if(isMobile() && is_dir("themes/{$_theme}_mobile")) $_theme .= "_mobile";
